...
class PaperDetector:
    ...
    def __init__(self, target_size: Tuple[int, int] = (1280, 720)):
        ...
        self.min_paper_area = 10000  # Minimum area to consider as paper (more lenient)
        self.max_paper_area = (
            5000000  # Maximum area to consider as paper (more lenient)
        )

        # Stabilization parameters (more lenient for real photos)
        self.stabilization_frames = 5  # Number of frames to average (reduced)
        self.detection_threshold = (
            0.5  # Minimum detection confidence to update (reduced)
        )
        ...
        self.update_interval = 1.0  # Minimum seconds between updates (reduced)
        self.min_stability_count = (
            2  # Minimum stable detections before updating (reduced)
        )
    ...
    def _find_best_paper_contour(
        ...
        for contour in contours:
            ...
            # Check if it's approximately rectangular (4-8 corners, more lenient)
            if 4 <= len(approx) <= 8:
                # Calculate rectangularity score
                rect_area = cv2.contourArea(approx)
                rect_score = rect_area / area if area > 0 else 0

                # Additional score for being close to target aspect ratio (more lenient)
                ...
                # Combined score (more weight on rectangularity, less on aspect ratio)
                score = rect_score * 0.8 + ratio_score * 0.2
