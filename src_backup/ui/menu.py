...
class KrathongScannerUI:
    ...
    def __init__(self):
        ...
        self.scanner_db = LocalTemplateRegistry("data/scanner.db")
    ...
    def run(self):
        ...
        self.root.geometry("700x750")  # Made taller for template selection section
    ...
    def create_main_interface(self):
        ...
        self.create_template_selection_section(main_frame)
    ...
    def create_template_selection_section(self, parent):
        ...
        # Template selection
        ttk.Label(template_frame, text="Available Templates:").grid(
            row=0, column=0, sticky=tk.W, padx=(0, 10)
        )

        self.template_var = tk.StringVar()
        self.template_combo = ttk.Combobox(
            template_frame, textvariable=self.template_var, state="readonly", width=40
        )
        ...
        refresh_btn = ttk.Button(
            template_frame, text="🔄 Refresh", command=self.load_available_templates
        )
        refresh_btn.grid(row=0, column=2, padx=(0, 10))

        # Template info
        self.template_info_var = tk.StringVar(value="No template selected")
        template_info_label = ttk.Label(
            template_frame, textvariable=self.template_info_var, foreground="gray"
        )
        template_info_label.grid(
    ...
    def load_available_templates(self):
        """Load available templates from scanner database."""
        try:
            self.available_templates = self.scanner_db.get_templates()

            # Filter only active templates
            active_templates = [t for t in self.available_templates if t.is_active]

            # Update combobox
            template_names = [
                f"{t.name} (IDs: {t.marker_ids})" for t in active_templates
            ]
            ...
            if template_names:
                ...
                self.template_info_var.set(
                    f"Loaded {len(active_templates)} active templates"
                )
            else:
                self.template_info_var.set("No active templates found in database")

        except Exception as e:
            self.template_info_var.set(f"Error loading templates: {e}")
    ...
    def on_template_selected(self, event=None):
        ...
        template_name = selection.split(" (IDs:")[0]
        ...
        if self.current_template:
            self.template_info_var.set(
                f"Selected: {self.current_template.name} - "
                f"Markers: {self.current_template.marker_ids}"
            )
        else:
            self.template_info_var.set("Template not found")

    def create_mode_buttons(self, parent):
        """Create the mode selection buttons."""
        # Button container
        button_frame = ttk.Frame(parent)
        button_frame.grid(row=3, column=0, columnspan=2, pady=(0, 30))
    ...
    def process_image_with_preview(self, file_path: str):
        ...
        try:
            ...
            # Check if detected markers match selected template
            detected_ids = [marker.id for marker in markers]
            template_marker_ids = set(self.current_template.marker_ids)
            detected_marker_ids = set(detected_ids)

            if not template_marker_ids.issubset(detected_marker_ids):
                missing_markers = template_marker_ids - detected_marker_ids
                raise ValueError(
                    f"Template markers not found. Missing: {list(missing_markers)}"
                )
            ...
            # Use template mask from database
            template_mask_path = Path(self.current_template.mask_path)
    ...
    def show_tunnel_instructions(self):
        ...
        instructions = (
            ...
            "• 24-hour sessions (vs other tunnel services' 2 hours)\n"
    ...
    def start_auto_directory_monitor(self, input_directory: str):
        ...
        try:
            ...
            detector = AutoDirectoryDetector(
                ...
                use_paper_detection=True,  # Enable paper detection
            )
