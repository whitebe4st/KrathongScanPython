...
class DocumentScanner:
    ...
    def find_document_contour(self, image):
        """Simple document detection - find any rectangular box in the paper"""

        # Try standard edge detection to find rectangular contours
        contour = self.find_contour_standard(image)
        if contour is not None:
            print("✓ Rectangle found using standard edge detection")
            return contour

        # Try adaptive threshold method
        contour = self.find_contour_adaptive(image)
        if contour is not None:
            print("✓ Rectangle found using adaptive threshold")
            return contour

        # Try morphological operations
        contour = self.find_contour_morphology(image)
        if contour is not None:
            print("✓ Rectangle found using morphological operations")
            return contour

        # Try multiple Canny thresholds
        contour = self.find_contour_multi_canny(image)
        if contour is not None:
            print("✓ Rectangle found using multiple Canny thresholds")
            return contour

        print("❌ Could not detect any rectangular region")
        return None

    def find_contour_standard(self, image):
        """Standard edge detection method to find rectangular regions"""
        try:
    ...
    def find_contour_adaptive(self, image):
        """Adaptive threshold method to find rectangular regions"""
        try:
            gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    ...
    def find_contour_morphology(self, image):
        """Morphological operations method to find rectangular regions"""
        try:
            gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    ...
    def find_contour_multi_canny(self, image):
        """Try multiple Canny threshold values to find rectangular regions"""
        try:
            gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
            blurred = cv2.GaussianBlur(gray, (5, 5), 0)
            ...
            pass
        return None
        """Ultra-precise paper boundary detection for tight zooming"""
        try:
            gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
            height, width = image.shape[:2]
            image_area = height * width

            print("Trying precise paper boundary detection for zoom...")

            # Method 1: Multi-stage edge detection with paper-specific filtering

            # Stage 1: Very heavy blur to eliminate ALL internal content
            super_blurred = cv2.GaussianBlur(gray, (101, 101), 0)  # Extreme blur

            # Stage 2: Adaptive thresholding on heavily blurred image
            # This should separate paper from background very cleanly
            ...
            # Stage 3: Find the largest white region (paper)
            contours, _ = cv2.findContours(
            ...
            if contours:
                ...
                # Paper should be at least 40% of image for precise detection
                if area > image_area * 0.4:
                    # Get tighter bounding contour
                    hull = cv2.convexHull(largest_contour)

                    # Use epsilon to get a clean rectangle
                    epsilon = 0.01 * cv2.arcLength(hull, True)  # Very precise
                    approx = cv2.approxPolyDP(hull, epsilon, True)

                    # If we get exactly 4 points, great!
                    if len(approx) == 4:
                        if not self.is_full_image(approx, width, height):
                            return approx

                    # Otherwise, force it to be a rectangle
                    rect = cv2.minAreaRect(hull)
                    box = cv2.boxPoints(rect)
                    rect_contour = np.array(box, dtype=np.int32).reshape(-1, 1, 2)

                    if not self.is_full_image(rect_contour, width, height):
                        return rect_contour

            # Method 2: Edge-based detection with paper-specific preprocessing

            # Apply bilateral filter to smooth content while preserving edges
            bilateral = cv2.bilateralFilter(gray, 15, 80, 80)

            # Very gentle edge detection to catch paper boundaries
            edges = cv2.Canny(bilateral, 15, 45, apertureSize=3, L2gradient=True)

            # Morphological operations to connect paper edge segments
            kernel_rect = cv2.getStructuringElement(cv2.MORPH_RECT, (7, 7))
            edges_closed = cv2.morphologyEx(edges, cv2.MORPH_CLOSE, kernel_rect)

            # Dilate to ensure paper edges are well connected
            edges_dilated = cv2.dilate(edges_closed, kernel_rect, iterations=2)
            ...
            # Process contours to find paper boundary
            for contour in sorted(contours, key=cv2.contourArea, reverse=True)[:3]:
                area = cv2.contourArea(contour)

                if area > image_area * 0.25:  # At least 25% of image
                    # Try multiple epsilon values for best fit
                    for epsilon_factor in [0.008, 0.012, 0.015, 0.02]:
                        ...
                        if len(approx) == 4:
                            if not self.is_full_image(approx, width, height):
                                # Additional check: ensure it's a reasonable rectangle
                                if self.is_valid_paper_rectangle(approx):
                                    return approx

                        elif 4 < len(approx) <= 8:
                            # Convert multi-point contour to rectangle
                            rect = cv2.minAreaRect(contour)
                            box = cv2.boxPoints(rect)
                            rect_approx = np.array(box, dtype=np.int32).reshape(
                                -1, 1, 2
                            )

                            if not self.is_full_image(rect_approx, width, height):
                                if self.is_valid_paper_rectangle(rect_approx):
                                    return rect_approx

            # Method 3: Otsu thresholding approach

            # Apply Gaussian blur
            blurred = cv2.GaussianBlur(gray, (21, 21), 0)

            # Otsu thresholding to separate paper from background
            _, otsu_thresh = cv2.threshold(
                blurred, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
            )

            # Find the largest white region (paper)
            contours, _ = cv2.findContours(
            ...
            if contours:
                ...
                if area > image_area * 0.3:  # At least 30% of image
                    # Get tight-fitting rectangle
                    rect = cv2.minAreaRect(largest_contour)
                    box = cv2.boxPoints(rect)
                    rect_contour = np.array(box, dtype=np.int32).reshape(-1, 1, 2)

                    if not self.is_full_image(rect_contour, width, height):
                        if self.is_valid_paper_rectangle(rect_contour):
                            return rect_contour

            print("❌ Precise boundary detection failed")
            return None

        except Exception as e:
            print(f"Error in precise boundary detection: {e}")
            return None

    def is_valid_paper_rectangle(self, contour):
        """Additional validation to ensure the contour represents a valid paper rectangle"""
        try:
            ...
            # Check aspect ratio - paper should be reasonably rectangular
            min_side = min(sides)
            max_side = max(sides)
            ...
            # Allow reasonable aspect ratios (not too extreme)
            if aspect_ratio > 5.0:  # Too thin/elongated
                return False

            # Check that opposite sides are roughly similar
            side1, side2, side3, side4 = sides

            # Calculate ratios of opposite sides
            ratio1 = (
            ...
            # Both ratios should be reasonably close (opposite sides similar)
            if ratio1 < 0.7 or ratio2 < 0.7:
                return False

            # Check that the rectangle isn't degenerate
            area = cv2.contourArea(contour)
            ...
            print(f"Error validating rectangle: {e}")
            return False
    ...
    def auto_scan_document(self):
        ...
        try:
            self.status_var.set("Detecting rectangular regions...")
            self.root.update()
            ...
            if contour is not None:
                # Apply perspective transformation to zoom into the detected rectangle
                self.status_var.set("Zooming into detected rectangle...")
                self.root.update()

                # Reshape contour for processing
                pts = contour.reshape(4, 2)
                print(f"Zooming into rectangle with corners: {pts}")

                # Apply perspective transform to extract and zoom the detected rectangle
                ...
                self.status_var.set("Rectangle detected and zoomed successfully!")

            else:
                # If no rectangle found, use the original image
                messagebox.showinfo(
                    "Info",
                    "Could not detect any rectangular region. Using original image.",
                )
    ...
    def visualize_detected_contour(self, contour):
        """Show what contour was detected on the original image with enhanced visualization"""
        try:
            # Create a copy of the original image for visualization
            vis_image = self.original_cv.copy()

            # Draw the detected contour in thick red line
            cv2.drawContours(vis_image, [contour], -1, (0, 0, 255), 5)

            # Draw corner points in green circles
            pts = contour.reshape(4, 2)
            for i, point in enumerate(pts):
                cv2.circle(vis_image, tuple(point.astype(int)), 12, (0, 255, 0), -1)
                # Add corner numbers
                cv2.putText(
                    vis_image,
                    str(i + 1),
            ...
            zoom_ratio = original_area / detected_area if detected_area > 0 else 1
            ...
            cv2.putText(
                vis_image,
                f"Area: {detected_area/original_area*100:.1f}% of image",
                (20, 80),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.6,
                (255, 255, 255),
            ...
            time.sleep(1.5)  # Show detection longer
