...
class KrathongScannerUI:
    ...
    def generate_qr_link(self):
        ...
        try:
            ...
            # Force a fresh tunnel check - don't rely on cached status
            tunnel_url = self.detect_tunnel_url(force_refresh=True)

            if tunnel_url:
                # Update public URL label
                self.public_url_var.set(f"Public URL: {tunnel_url}")
                # Generate QR code
                    ...
                    if result:
                        ...
                        try:
                            if os.name == "nt":  # Windows
                                # Use subprocess with CREATE_NO_WINDOW to avoid CMD pop-up
                                import subprocess
    ...
    def detect_tunnel_url(self, force_refresh=False):
                    ...
                    try:
                        ...
                        if status_response.status_code == 200:
                            ...
                            if public_url:
                                ...
                                # If force refresh is requested, test the connection and restart if needed
                                if force_refresh:
                                    try:
                                        ...
                                        if (
                                            tunnel_response.status_code == 200
                                            and "Tunnel not connected"
                                            not in tunnel_response.text
                                        ):
                                            print(
                                            ...
                                            print(
                                                f"⚠️ Tunnel not responding or stale, will restart"
                                            )
                                    except:
                                        print(
                                            f"⚠️ Tunnel connection failed, will restart"
                                        )
                                else:
                                    # For normal checks, just verify tunnel is still working
                                    try:
                                        tunnel_response = requests.get(
                                            public_url, timeout=10
                                        )
                                        if (
                                            ...
                                            print(
                                                f"✅ Found existing tunnel URL: {public_url}"
                                            )
                                            return public_url
                                        else:
                                            print(
    ...
    def run_web_server_thread(self):
        ...
        try:
            ...
            # Update status
            self.show_status("Starting Flask server...")
            ...
            # Wait for Flask server to be actually ready
            import requests

            self.show_status("⏱️ Waiting for Flask server to be ready...")
            flask_ready = False
            ...
            for attempt in range(max_wait_time):
                try:
                    ...
                    if response.status_code == 200:
                        self.show_status(
                            f"✅ Flask server ready after {attempt + 1} seconds!"
                        )
                        flask_ready = True
                        break
                except requests.exceptions.RequestException:
            ...
            if not flask_ready:
                self.show_status("❌ Flask server failed to start!")
                raise Exception("Flask server startup timeout")

            # Update status
            self.show_status("Web server running on http://localhost:5000")

            # Now start InstaTunnel after Flask is confirmed ready
            self.show_status("Starting InstaTunnel for public access...")
            public_url = server.start_instatunnel(5000)
            if public_url:
                self.show_status(f"Public URL: {public_url}")
            else:
                self.show_status(
                    "InstaTunnel failed to start - using local access only"
                )

            # Keep the web server thread alive (the Flask thread is daemon)
            try:
                while True:
                    time.sleep(1)
            except:
