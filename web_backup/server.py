...
# Custom error handler for file size too large
@app.errorhandler(413)
def too_large(e):
    response = jsonify(
        {
            "error": "File too large. Maximum size is 100MB. Please compress your image and try again."
        }
    )
...
def setup_auto_detector():
    ...
    if auto_detector is None:
        auto_detector = AutoDirectoryDetector(
            ...
            use_homography=True,  # Keep homography enabled
        )
...
def upload_file():
    ...
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            print(
                f"📁 Uploading file: {file.filename}, Size: {file_size / (1024*1024):.2f} MB"
            )
            ...
            if file_size > 500 * 1024:  # 500KB
                ...
                try:
                    ...
                    # Compress the image (reduce quality to 60% for better compression)
                    encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), 60]
                    _, compressed_data = cv2.imencode(".jpg", cv_image, encode_param)

                    # Create new file-like object with compressed data
                    compressed_file = io.BytesIO(compressed_data.tobytes())
                    compressed_file.filename = (
                        file.filename
                    )  # Fix: use .filename instead of .name

                    print(
                        f"✅ Image compressed: {len(compressed_data.tobytes()) / (1024*1024):.2f} MB"
                    )
            ...
            if hasattr(file, "save"):
                # Original file object (has save method)
                file.save(filepath)
            else:
                # Compressed BytesIO object (write directly)
...
def check_status(job_id):
    ...
    try:
        ...
        if job.status == "completed" and job.result_file:
            ...
            response["preview_url"] = url_for(
                "preview_result", filename=job.result_file
            )
...
def start_instatunnel(port):
    ...
    try:
        ...
        print(f"🔍 Command: {instatunnel_cmd} {port} --subdomain {custom_subdomain}")
        ...
        start_time = time.time()
        timeout = 30  # 30 seconds timeout for InstaTunnel
        ...
        while time.time() - start_time < timeout:
            ...
            try:
                ...
                if line:
                    ...
                    # InstaTunnel outputs: "🚀 Your app is now live at https://subdomain.instatunnel.my"
                    ...
                    # Also check for any InstaTunnel URL pattern
                    if "instatunnel.my" in line:
                        import re

                        url_match = re.search(r"https://[^\s]+instatunnel\.my", line)
                        if url_match:
                            tunnel_url = url_match.group(0)
                            public_url = tunnel_url
                            print(f"✅ InstaTunnel started: {tunnel_url}")
        ...
        # Fallback: Try to get URL from InstaTunnel status
        try:
            print("🔍 Trying fallback method to get InstaTunnel URL...")
            # Check if InstaTunnel is still running
            if process.poll() is None:
                print("✅ InstaTunnel process is still running")
                # Try to get the URL from the subdomain we generated
                fallback_url = f"https://{custom_subdomain}.instatunnel.my"
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        print(f"✅ InstaTunnel fallback URL works: {fallback_url}")
                        return fallback_url
                except:
                    pass
